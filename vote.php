...
if (!$error) {
    try {
        ...
        $sql_positions_candidates = "SELECT p.id as position_id, p.title as position_title, c.id as candidate_id, c.name as candidate_name, c.photo as candidate_photo, $classSectionSelect FROM positions p LEFT JOIN candidates c ON p.id = c.position_id WHERE p.election_id = $election_id ORDER BY p.id ASC, c.name ASC";
                                ...
                                <div class="candidate-card <?php echo $is_checked ? 'selected' : ''; ?>" tabindex="0" role="radio" aria-checked="<?php echo $is_checked ? 'true' : 'false'; ?>" aria-labelledby="candidate-name-<?php echo $candidate['id']; ?>">
                                    <label class="candidate-option d-flex flex-column text-decoration-none p-0 h-100">
                                        ...
                                        <div class="candidate-info">
                                            <h3 class="candidate-name" id="candidate-name-<?php echo $candidate['id']; ?>"><?php echo htmlspecialchars($candidate['name']); ?><?php echo candidate_class_section_display_html($candidate['class_section'] ?? ''); ?></h3>
                                        </div>
                                    </label>
                                </div> <?php endforeach; ?>
                            </div>
